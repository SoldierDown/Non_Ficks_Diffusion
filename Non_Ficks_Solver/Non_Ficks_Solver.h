//!#####################################################################
//! \file Non_Ficks_Solver.h
//!#####################################################################
// Class Non_Ficks_Solver
//######################################################################
#ifndef __Non_Ficks_Solver__
#define __Non_Ficks_Solver__
...
class Non_Ficks_Solver
{
  ...
  public:
    ...
    Non_Ficks_Solver(Hierarchy_Base& hierarchy_input,const int mg_levels,const T coeff1_input)
        :hierarchy(hierarchy_input),coeff1(coeff1_input)
    {
    ...
    {
        Non_Ficks_Smoother<Multigrid_struct_type,T,d>::Jacobi_Iteration(*multigrid_hierarchy(mg_level),multigrid_hierarchy(mg_level)->Boundary_Blocks(mg_level),
                                                                    mg_level,u_channel,b_channel,temp_channel,coeff1,boundary_smoothing_iterations,(unsigned)MG_Boundary);
        Non_Ficks_Smoother<Multigrid_struct_type,T,d>::Jacobi_Iteration(*multigrid_hierarchy(mg_level),multigrid_hierarchy(mg_level)->Blocks(mg_level),
                                                                    mg_level,u_channel,b_channel,temp_channel,coeff1,interior_smoothing_iterations,(unsigned)Cell_Type_Interior);
        Non_Ficks_Smoother<Multigrid_struct_type,T,d>::Jacobi_Iteration(*multigrid_hierarchy(mg_level),multigrid_hierarchy(mg_level)->Boundary_Blocks(mg_level),
                                                                    mg_level,u_channel,b_channel,temp_channel,coeff1,boundary_smoothing_iterations,(unsigned)MG_Boundary);
    }
    ...
    void V_Cycle(const int boundary_smoothing_iterations,const int interior_smoothing_iterations,const int bottom_smoothing_iterations) const
    {
        ...
        // exact solve
        Non_Ficks_Smoother<Multigrid_struct_type,T,d>::Exact_Solve(*multigrid_hierarchy(mg_levels-1),u_channel,b_channel,temp_channel,
                                                               coeff1,bottom_smoothing_iterations,(unsigned)Cell_Type_Interior);
    ...
    {
        Non_Ficks_Smoother<Multigrid_struct_type,T,d>::Compute_Residual(*multigrid_hierarchy(mg_level),u_channel,b_channel,
                                                                    temp_channel,coeff1,(unsigned)Cell_Type_Interior);
    }
