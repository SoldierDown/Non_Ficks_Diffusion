...
class Epsilon_Squared_Grad_S_Helper
{
  ...
  public:
    Epsilon_Squared_Grad_S_Helper(Allocator_type& allocator,const std::pair<const uint64_t*,unsigned>& blocks,Channel_Vector& face_qp_channels,
                   T Struct_type::* epsilon_channel,T Struct_type::* density_backup_channel,const T minus_dt_over_tau_1,const T one_over_dx,const int level)
    {Run(allocator,blocks,face_qp_channels,epsilon_channel,density_backup_channel,minus_dt_over_tau_1,one_over_dx,level);}

    void Run(Allocator_type& allocator,const std::pair<const uint64_t*,unsigned>& blocks,Channel_Vector& face_qp_channels,
            T Struct_type::* epsilon_channel,T Struct_type::* density_backup_channel,const T minus_dt_over_tau_1,const T one_over_dx,const int level) const
    {
        ...
        auto epsilon_squared_grad_s_helper=[&](uint64_t offset)
        {
            for(unsigned e=0;e<Flag_array_mask::elements_per_block;++e,offset+=sizeof(Flags_type)) for(int axis=0;axis<d;++axis){
                ...
                if(flags(offset)&Topology_Helper::Face_Active_Mask(axis)){uint64_t neighbor_offset=Flag_array_mask::Packed_Add(offset,negative_face_offsets(axis));
                    ...
                    face_qp_data(offset)+=minus_dt_over_tau_1*Nova_Utilities::Sqr(interpolated_epsilon_value)*(density_backup_data(offset)-density_backup_data(neighbor_offset))*one_over_dx;}}
        };
