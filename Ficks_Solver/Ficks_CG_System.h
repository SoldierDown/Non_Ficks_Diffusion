//!#####################################################################
//! \file Ficks_CG_System.h
//!#####################################################################
// Class Ficks_CG_System
//######################################################################
#ifndef __Ficks_CG_System__
#define __Ficks_CG_System__
...
class Ficks_CG_System: public Krylov_System_Base<T>
{
  ...
  public:
    ...
    const T Ddt;
    Ficks_CG_System(Hierarchy& hierarchy_input,const int mg_levels_input,const int boundary_smoothing_iterations_input,
                    const int interior_smoothing_iterations_input,const int bottom_smoothing_iterations_input,const T Ddt_input)
        ...
        interior_smoothing_iterations(interior_smoothing_iterations_input),bottom_smoothing_iterations(bottom_smoothing_iterations_input),Ddt(Ddt_input)
    {
    ...
    void Multiply(const Vector_Base& v,Vector_Base& result) const
    {
        ...
        Ficks_Smoother<Base_struct_type,T,d>::Multiply_With_System_Matrix(hierarchy,v_channel,result_channel,Ddt);
    }
