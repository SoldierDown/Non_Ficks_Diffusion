...
class Standard_Tests: public MPM_Example<T,d>
{
  ...
  public:
    ...
    using Base::mg_levels; using Base::n_solid; using Base::n_fluid; using Base::nbw; using Base::fluid_source;
...
//######################################################################
    void Parse_Options() override
    {
        ...
        output_directory=std::to_string(d)+"d_"+(FICKS?"F_":"NF_")+"E_"+std::to_string(E)+"_nu_"+std::to_string(nu)+"_diff_"+std::to_string(diff_coeff)
                                +"_eta_"+std::to_string(eta)+"_Fc_"+std::to_string(Fc)+"_tau_"+std::to_string(tau)+"_Resolution_"+std::to_string(counts(0))+"_"
                                +std::to_string(mg_levels)+"_levels_"+std::to_string(n_solid)+"_hydrogel_particles_"+std::to_string(n_fluid)+"_fluid_particles";
    ...
    break;
        ...
        case 23:{
            ...
            // fluid
            {
                const T solid_density=(T)10.;
                const T fluid_density=(T)1.;
                const int number_of_particles=n_fluid;
