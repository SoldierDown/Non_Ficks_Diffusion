...
class Uniform_Grid_Density_Advection_Helper
{
  ...
  public:
    Uniform_Grid_Density_Advection_Helper(Hierarchy& hierarchy,const std::pair<const uint64_t*,unsigned>& blocks,T Struct_type::* cell_channel,
                                T Struct_type::* result_channel,Channel_Vector& node_velocity_channels,uint64_t* nodes_of_cell_offsets,
                                const int level,const T dt,const unsigned mask)
    {Run(hierarchy,blocks,cell_channel,result_channel,node_velocity_channels,nodes_of_cell_offsets,level,dt,mask);}

    void Run(Hierarchy& hierarchy,const std::pair<const uint64_t*,unsigned>& blocks,T Struct_type::* cell_channel,T Struct_type::* result_channel,
                Channel_Vector& node_velocity_channels,uint64_t* nodes_of_cell_offsets,const int level,const T dt,const unsigned mask) const
    {
