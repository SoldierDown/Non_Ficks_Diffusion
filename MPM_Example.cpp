...
//######################################################################
template<class T,int d> int MPM_Example<T,d>::
ID321(Vector<int,d> index)
{
    // int id=0;
    // for(int v=0;v<d;++v) id+=index(v)*dn(v);
    return 0;
}
//######################################################################
// Update_Constitutive_Model_State
...
Parse_Options()
{
    ...
    // for(int i=0;i<d;++i){dn(i)=1;
    //     for(int axis=i+1;axis<d;++axis) dn(i)*=counts(axis)+1;}
    //     Log::cout<<dn<<std::endl;
}
//######################################################################
// Write_Output_Files
//######################################################################
