        ...
        if(File_Utilities::File_Exists(directory_name+"/common/",false))
        {
            ...
            BOOST_FOREACH(fs::path const &p,std::make_pair(it,eod)) if(fs::is_regular_file(p)){std::string filename(p.c_str());
                if(filename.find_last_of('.')!=std::string::npos){
                    ...
                    if(extension=="mpm3d"){
                        renderables.push_back(std::move(std::unique_ptr<Renderable>(new Nova::MPM_Renderable<float,3>(app,directory_name,number_of_frames))));
                        dynamic_cast<Simulation_Renderable<float,3>*>(renderables.back().get())->Load_Active_Frame();}
                    else if(extension=="mpm2d"){
                        renderables.push_back(std::move(std::unique_ptr<Renderable>(new Nova::MPM_Renderable<float,2>(app,directory_name,number_of_frames))));
