//!#####################################################################
//! \file Explicit_Face_Qc_Updater.h
//!#####################################################################
// Class Explicit_Face_Qc_Updater
//######################################################################
#ifndef __Explicit_Face_Qc_Updater__
#define __Explicit_Face_Qc_Updater__
...
class Explicit_Face_Qc_Updater
{
  ...
  public:
    Explicit_Face_Qc_Updater(Hierarchy& hierarchy,const std::pair<const uint64_t*,unsigned>& blocks,Channel_Vector& face_qc_channels,Channel_Vector& face_qc_backup_channels,
                   ...
                   T Struct_type::* density_channel,const T coeff1,const T coeff2,const int level) const
    {
        ...
        auto explicit_face_qc_updater=[&](uint64_t offset)
        {
        ...
        SPGrid_Computations::Run_Parallel_Blocks(blocks,explicit_face_qc_updater);
    }
};
}
#endif
