...
class Add_Random_Term
{
  ...
  public:
    Add_Random_Term(Allocator_type& allocator,const std::pair<const uint64_t*,unsigned>& blocks,
                                    ...
                                    const T alpha_dt_over_tau_s,const T random_value)
    {Run(allocator,blocks,density_channel,density_backup_channel,alpha_dt_over_tau_s,random_value);}

    void Run(Allocator_type& allocator,const std::pair<const uint64_t*,unsigned>& blocks,                                    
            T Struct_type::* density_channel,T Struct_type::* density_backup_channel,
            const T alpha_dt_over_tau_s,const T random_value) const
    {
            ...
            for(int e=0;e<Flag_array_mask::elements_per_block;++e,offset+=sizeof(Flags_type))
                if(flags(offset)&Cell_Type_Interior){ const T density_value=density_backup_data(offset);
                     if(density_value>(T)0. && density_value<=(T).5) 
                        density_data(offset)+=alpha_dt_over_tau_s*density_value*((T)1.-density_value)*(random_value-(T).5);}
        };
